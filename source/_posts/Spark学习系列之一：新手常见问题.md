...
我相信一开始接触的初学者对此肯定有疑惑，并行度指的什么？我认为在spark中，这个并行度指的是partition的数量，无论是通过parallelize初始化rdd，还是通过join和reduceByKey等shuffle操作，都意味着需要确定这个新rdd的paritition数量。这里涉及到一个参数`spark.default.parallelism`，这个__动态参数__是parallelize、join、reducdeByKey等操作的__默认__并行度，这个参数在不同情景的情况下，__默认值__有所不同：

- 对于join和reduceByKey等shuffl操作，该参数的默认值为多个父rdd中partition数目最大的一个。
- 对于parallelize进行初始化操作，该参数默认值在不同部署模式下不同：
	- local[*]：本地cpu的core数量，local[N]则为N，local则为1
	- meos：默认为8
	- other：executor个数 * 每个executor的core个数
- 这里需要额外注意的是`spark.default.parallelism`是个 __动态__ 参数，在不同场景下是会变的，并且它是并行度的默认值，实际api中也能通过传递numPartitions参数覆盖`spark.default.parallelism`，自行决定并行度。
